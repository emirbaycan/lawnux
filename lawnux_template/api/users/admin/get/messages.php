...
if (isset($data->search)) {
    $m = array('fullname','email','phone','message','country','city','created_at');
    $m4 = $data->search;
    foreach ($m as $m2) {
        $m3[] = $m2.' LIKE CONCAT("%",?,"%")';
        $params[] = $m4;
...
if (isset($data->order)) {
    $m = array('fullname','email','phone','subject','message','b.country','b.city','created_at','a.id');
    $m2 = array('asc','desc');
    $m3 = $data->order;
    $m5 = [];
    foreach ($m3 as $m4=>$m6) {
...
$query = 'SELECT 
a.*,b.ip,b.country,b.city,b.user_agent
FROM contact a inner join
contact_infos b on a.id = b.id
'.$where.$order.' LIMIT ?,?';
$counter = 'SELECT
